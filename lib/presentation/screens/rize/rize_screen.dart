...
class RizeScreen extends StatefulWidget {
  ...
  @override State<RizeScreen> createState() => _RizeScreenState();
}
      ...
      child: Scaffold(
        ...
        body: SafeArea(child: Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              Text('Rize', style: AppTextStyles.h4.copyWith(
                color: AppColors.white, fontWeight: FontWeight.w900)),
              const Spacer(),
              const Icon(Icons.tune_rounded, color: AppColors.white, size: 22),
            ]),
          ),
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      separatorBuilder: (_, __) => Divider(
        color: AppColors.grey.withOpacity(0.3), height: 1),
      itemBuilder: (_, i) => _RizeCard(
        post: posts[i],
        onUpdate: (p) => onUpdate(i, p),
        onTap: () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => _RizeDetailScreen(post: posts[i]))),
      ),
    );
  }
}

// ─── Rize Card (Threads style) — 650×1000px media ─────────────────────────────
class _RizeCard extends StatelessWidget {
  final RizePostModel post;
  final Function(RizePostModel) onUpdate;
  final VoidCallback onTap;
  const _RizeCard({required this.post, required this.onUpdate, required this.onTap});
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(post.name, style: AppTextStyles.labelLarge.copyWith(
                color: AppColors.white, fontWeight: FontWeight.bold)),
              Text('${post.username} · ${Formatters.timeAgo(post.createdAt)}',
                style: AppTextStyles.labelSmall.copyWith(color: AppColors.grey2)),
            ])),
            const Icon(Icons.more_horiz_rounded, color: AppColors.grey2, size: 20),
          ]),
          const SizedBox(height: 10),
          // Title
          Text(post.title, style: AppTextStyles.labelLarge.copyWith(
            color: AppColors.white, fontWeight: FontWeight.w700)),
          const SizedBox(height: 6),
          // Body (truncated)
          Text(post.body,
            style: AppTextStyles.body2.copyWith(color: Colors.white70, height: 1.45),
            maxLines: 3, overflow: TextOverflow.ellipsis),
          // Media — 650×1000 aspect ratio مستطيل عمودي
          if (post.hasMedia) ...[
            const SizedBox(height: 10),
            ClipRRect(
              ...
              child: Container(
                ...
                height: (MediaQuery.of(context).size.width - 28) / 0.65,
          ...
          const SizedBox(height: 10),
          // Actions — L shape: upvote | comment | share | bookmark
          Row(children: [
            _upvoteBtn(),
            const SizedBox(width: 16),
            _actionBtn(Icons.chat_bubble_outline_rounded,
              Formatters.formatCount(post.comments), AppColors.grey2, () {}),
            const SizedBox(width: 16),
            _actionBtn(Icons.repeat_rounded,
              Formatters.formatCount(post.shares), AppColors.grey2, () {}),
            const SizedBox(width: 16),
            _actionBtn(Icons.send_outlined,
              '', AppColors.grey2, () {}),
            const Spacer(),
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: post.isUpvoted
              ? AppColors.neonYellow.withOpacity(0.15) : Colors.transparent,
          border: Border.all(
            color: post.isUpvoted ? AppColors.neonYellow : AppColors.grey2.withOpacity(0.4),
            width: 1.5),
          borderRadius: BorderRadius.circular(20)),
        child: Row(children: [
          Icon(Icons.arrow_upward_rounded,
            color: post.isUpvoted ? AppColors.neonYellow : AppColors.grey2, size: 16),
          const SizedBox(width: 4),
          Text(Formatters.formatCount(post.upvotes),
            style: AppTextStyles.labelSmall.copyWith(
              color: post.isUpvoted ? AppColors.neonYellow : AppColors.grey2,
...
// ─── Rize Detail (تفتح الفيديو/صورة في فيد كامل) ────────────────────────────
class _RizeDetailScreen extends StatefulWidget {
  final RizePostModel post;
  const _RizeDetailScreen({required this.post});
  @override State<_RizeDetailScreen> createState() => _RizeDetailScreenState();
}
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              if (!_isPlaying)
                const Icon(Icons.play_arrow_rounded, color: Colors.white54, size: 80)
              else
                const Icon(Icons.pause_rounded, color: Colors.transparent, size: 80),
            ]),
          ),
        ),
        // Gradient overlay
        Positioned(bottom: 0, left: 0, right: 0, height: size.height * 0.4,
          child: Container(
            decoration: BoxDecoration(
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('New Rize', style: AppTextStyles.h5.copyWith(
          color: AppColors.white, fontWeight: FontWeight.w900)),
        const SizedBox(height: 16),
        TextField(
          ...
          decoration: InputDecoration(
            ...
            filled: true, fillColor: AppColors.grey,
        ...
        Row(children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Text('Post', style: AppTextStyles.labelLarge.copyWith(
                color: AppColors.white, fontWeight: FontWeight.bold)))),
        ]),
        const SizedBox(height: 16),
      ]),
  ...
  Widget _mediaBtn(BuildContext ctx, IconData icon, String label) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          Icon(icon, color: AppColors.grey2, size: 18),
